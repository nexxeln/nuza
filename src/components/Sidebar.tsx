...
function FileNode({ entry, onFileSelect }: { entry: FileEntry; onFileSelect?: (path: string) => void }) {
    ...
    return (
      <li>
        <details className="group">
          ...
          <ul className="ml-5 border-l border-zinc-700 pl-3 cursor-pointer">
            {entry.children?.map((child) => (
              <FileNode key={child.path} entry={child} onFileSelect={onFileSelect} />
            ))}
          </ul>
        </details>
      </li>
  ...
  return (
    <li>
      <button 
        onClick={() => onFileSelect && onFileSelect(entry.path)}
        className="flex w-full items-center gap-1.5 rounded-md pl-7 pr-2 py-1.5 text-left text-sm text-zinc-400 transition-colors hover:bg-zinc-800/50 hover:text-zinc-100 cursor-pointer"
      >
...
export default function Sidebar({ data, onOpenFolder, onFileSelect }: { data: FileEntry[]; onOpenFolder?: () => void; onFileSelect?: (path: string) => void }) {
  return (
